...
well-observed nebular body. We further validate the map with a web-based
perceptual survey ($n=9$ raters, 14 patches spanning the observed
uncertainty range): while model uncertainty does not significantly
correlate with the mean human "naturalness" rating, it correlates
significantly with the degree of \emph{disagreement among raters}
(Spearman $\rho=0.59$, $p=0.03$), suggesting that regions the model flags
as uncertain are also regions that are genuinely ambiguous to independent
human observers. All code is released publicly to support reproduction and
...
\subsection{Perceptual survey results}
\label{sec:survey-results}

Nine raters completed the survey ($n=9$; one rater skipped a single patch,
giving $n=8$ for that item). Two complementary correlations were tested
against the model's mean per-patch uncertainty using Spearman's rank
correlation coefficient: (i) the mean human "naturalness" rating per patch,
and (ii) the standard deviation of ratings across raters for that patch,
i.e., the degree of \emph{inter-rater disagreement}.

The correlation between model uncertainty and mean human rating was not
statistically significant ($\rho = -0.227$, $p = 0.435$): patches with
higher model uncertainty were not consistently rated as less natural on
average. However, the correlation between model uncertainty and
\emph{inter-rater disagreement} was positive and statistically significant
($\rho = 0.591$, $p = 0.026$, $n=14$ patches). That is, patches on which the
model was more uncertain were also patches on which human raters disagreed
more strongly with one another about naturalness (Figure~\ref{fig:survey}).
The patch with the single highest model uncertainty (containing a saturated
foreground star, roughly $3\times$ the uncertainty of any other patch) also
had among the highest inter-rater standard deviations in the entire set.

We consider this a more informative and more robust signal than the
mean-rating correlation for a dataset of this size: the mean rating of a
single rater is a noisy point estimate of an inherently subjective
quantity, whereas disagreement \emph{among} several independent raters is a
direct, model-free proxy for how ambiguous a patch actually is --- which is
conceptually close to what an epistemic uncertainty estimate is intended to
capture. The result suggests that regions the network flags as uncertain
correspond to regions that are also genuinely ambiguous to independent
human observers, even though it does not yet establish that the model's
uncertainty predicts the \emph{direction} (more vs.\ less trustworthy) of
that ambiguity.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{survey_correlation.png}
  \caption{Left: mean human naturalness rating (1--5) vs.\ model
  uncertainty per patch; the correlation is not significant
  ($\rho=-0.23$, $p=0.44$). Right: standard deviation of human ratings
  (inter-rater disagreement) vs.\ model uncertainty per patch; the
  correlation is significant ($\rho=0.59$, $p=0.03$). $n=9$ raters,
  14 patches.}
  \label{fig:survey}
\end{figure}

We note two caveats. First, with $n=9$ raters and 14 patches, this result
should be treated as an initial finding rather than a definitive validation;
we plan to continue data collection toward $n \geq 20$ raters to obtain a
more stable estimate of both effects and to test whether the mean-rating
correlation strengthens with a larger, less noisy sample. Second, the
disagreement metric is itself noisy at $n=9$ per patch; a pre-registered
replication with a larger, independent rater pool would strengthen this
result considerably.
...
statistical confidence interval on true physical flux. Second, while the
disagreement-based correlation reported in Section~\ref{sec:survey-results}
is statistically significant, it is based on only 9 raters and 14 patches
from a single target; the mean-rating correlation remains non-significant
at this sample size, and both results should be treated as preliminary
pending a larger, pre-registered replication. Third, results are
demonstrated on a single target (M16); generalization to other nebulae,
filter sets, and instruments (e.g., JWST NIRCam) remains to be tested.
...
perceptual survey instrument for human validation. In an initial survey of
9 raters, model uncertainty did not significantly predict mean human
naturalness ratings, but did significantly predict \emph{inter-rater
disagreement} ($\rho=0.59$, $p=0.03$), suggesting the model's uncertainty
estimate captures genuine perceptual ambiguity. Immediate next steps are
(1) expanding the survey toward $n \geq 20$ raters to test whether this
result strengthens and whether the mean-rating effect becomes detectable,
(2) extending the pipeline to additional targets and instruments, and (3)
exploring alternative uncertainty-estimation techniques (deep ensembles,
evidential deep learning) as a comparison point for MC-Dropout. All code,
the survey instrument, and this manuscript source are released publicly
(see repository).
